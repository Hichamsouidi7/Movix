...
// Proxy endpoint to strip X-Frame-Options and CSP for embedded web portals (YouTube, Twitch, etc.)
app.get('/api/iframe-proxy', async (req, res) => {
  const targetUrl = req.query.url || 'https://www.youtube.com';
  try {
...
// Wildcard Reverse Proxy for Full YouTube Site (Strips X-Frame-Options & CSP for all sub-resources)
app.use('/api/yt-proxy', (req, res) => {
  const targetHost = 'www.youtube.com';
  ...
  proxyReq.on('error', (err) => {
    console.error('YouTube reverse proxy error:', err.message);
    if (!res.headersSent) res.status(502).send('YouTube Proxy Error: ' + err.message);
  });
