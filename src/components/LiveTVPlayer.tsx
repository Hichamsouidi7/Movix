...
import { findIptvOrgChannelById, findIptvOrgChannelByName } from '../utils/iptvOrgFrance';
import { FRENCH_TV_CHANNELS } from '../data/frenchTvChannels';
...
// Constante pour le serveur proxy embed (fallback sur MAIN_API si localhost:25569 est inaccessible)
const PROXIES_EMBED_API_URL = (PROXIES_EMBED_API && !PROXIES_EMBED_API.includes('localhost:25569'))
    ? PROXIES_EMBED_API
    : 'https://proxiesembed.movix.show';
...
const LiveTVPlayer: React.FC<LiveTVPlayerProps> = ({
    ...
    const fetchChannelPayload = useCallback(async (requestOptions: StreamRequestOptions = {}) => {
        if (channelId.startsWith('iptvorg_')) {
            const channel = findIptvOrgChannelById(channelId);
            if (channel) {
                return {
                    streams: [
                        {
        ...
        const endpoints = [
            `${API_BASE}/api/livetv/stream/tv/${channelId}`,
            `https://api.movix.show/api/livetv/stream/tv/${channelId}`,
            `http://localhost:25565/api/livetv/stream/tv/${channelId}`
        ];
        let response: Response | null = null;
        for (const ep of endpoints) {
            try {
                const requestUrl = new URL(ep);
        ...
        if (!response || !response.ok) {
            ...
            if (response) httpError.status = response.status;
            throw httpError;
        }
    ...
    const loadResolvedStreams = useCallback(async (requestOptions: StreamRequestOptions = {}) => {
        ...
        const cleanId = channelId.replace(/^(vavoo_|iptvorg_|wiflix_|daddylive_)/, '');
        const frenchChannel = FRENCH_TV_CHANNELS.find(c =>
            c.catalogId === channelId ||
            c.catalogId.replace('vavoo_', '') === cleanId ||
            c.name.toLowerCase() === cleanId.toLowerCase()
        );
        const searchName = frenchChannel?.name || cleanId;
        const iptvOrgMatch = findIptvOrgChannelByName(searchName);
        ...
        try {
            ...
            for (let attempt = 0; attempt < MAX_404_RETRIES && !loaded; attempt++) {
                try {
                    ...
                    combinedStreams.push(...validStreams);
            ...
            // Append fallback streams if available
            if (frenchChannel?.fallbackUrl && !combinedStreams.some(s => s.url === frenchChannel.fallbackUrl)) {
                combinedStreams.push({
                    title: `${frenchChannel.name} (Direct / Secours)`,
                    url: frenchChannel.fallbackUrl,
                    _directPlay: true
                });
            }

            if (iptvOrgMatch?.url && !combinedStreams.some(s => s.url === iptvOrgMatch.url)) {
                combinedStreams.push({
                    title: `${iptvOrgMatch.name} (IPTV-Org)`,
                    url: iptvOrgMatch.url,
                    _directPlay: true
                });
            }
    ...
    }, [MAX_404_RETRIES, RETRY_458_INTERVAL_MS, RETRY_INTERVAL_MS, channelId, fetchChannelPayload, t]);
        ...
        if (isMpegTs) {
            ...
            const hlsConfig: any = {
                ...
                xhrSetup: (xhr: XMLHttpRequest, url: string) => {
                    ...
                    if (isVavooStream || urlLower.includes('/sunshine/') || urlLower.includes('vavoo') || urlLower.includes('ngolpdkyoctjcddxshli469r')) {
                        try {
    ...
    return (
        ...
        >
            {/* Close button - visible pour tous les flux (HLS ou embed) */}
            <AnimatePresence>
                {(showControls || isEmbedStream) && (
                    <motion.button
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                        className="absolute top-4 left-4 z-[50000] flex items-center gap-2 px-3.5 py-2 bg-black/80 hover:bg-black text-white rounded-xl border border-white/20 shadow-2xl backdrop-blur-md transition-all cursor-pointer"
                        title="Quitter (Retour)"
                    >
                        <ArrowLeft size={18} className="text-red-500" />
                        <span className="font-semibold text-sm">Quitter</span>
                    </motion.button>
                )}
            </AnimatePresence>
