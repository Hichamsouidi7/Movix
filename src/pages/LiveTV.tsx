...
import { IPTV_ORG_CATALOGS, getIptvOrgChannelsByCatalog } from '../utils/iptvOrgFrance';
...
const sourceDisplayNames: { [key: string]: string } = {
  'iptvorg': '🇫🇷 TV Française (IPTV-Org)',
  'vavoo': 'TV Direct (Vavoo / TNT / Sport)',
...
const getSourceKey = (catalogId: string): string => {
  if (catalogId.startsWith('iptvorg_')) return 'iptvorg';
...
const LiveTV: React.FC = () => {
  ...
  const [selectedSource, setSelectedSource] = useState<string>('vavoo'); // Default source
  ...
  // Get unique sources from catalogs (filter out disabled/removed sources: livetv, daddylive, iptv)
  const disabledSourceKeys = new Set(['other', 'livetv', 'daddylive', 'iptv']);
  const availableSources = [...new Set(catalogs.map(c => getSourceKey(c.id)))]
    .filter(s => !disabledSourceKeys.has(s))
  ...
  const filteredChannels = channels.filter(channel => {
    ...
    // Masquer les chaînes étrangères explicites si on n'est pas dans un filtre spécifique
    const lowerName = name.toLowerCase();
    const foreignTags = [
      ' (us)', ' (uk)', ' (de)', ' (es)', ' (it)', ' (pt)', ' (pl)', ' (tr)', ' (ru)',
      ' (nl)', ' (gr)', ' (ro)', ' (bg)', ' (br)', ' (mx)', ' (ca)', ' (au)',
      ' germany', ' italia', ' espana', ' portugal', ' poland', ' turkey', ' russia', ' netherlands'
    ];
    if (foreignTags.some(tag => lowerName.includes(tag))) {
      return false;
    }
  ...
  const fetchCatalogChannelsById = useCallback(async (catalogId: string): Promise<Channel[]> => {
    ...
    if (catalogId.startsWith('iptvorg_')) {
      const orgChannels = await getIptvOrgChannelsByCatalog(catalogId);
      return orgChannels.map(c => ({
        id: c.id,
        type: 'tv',
        name: c.name,
        poster: c.poster || '/movix.png',
      }));
    }
    ...
    if (isExtensionAvailable()) {
      ...
      const endpoints = [
        `${API_BASE}/api/livetv/catalog/tv/${catalogId}`,
        `https://api.movix.show/api/livetv/catalog/tv/${catalogId}`,
        `http://localhost:25565/api/livetv/catalog/tv/${catalogId}`
      ];
      for (const endpoint of endpoints) {
        try {
          ...
          if (response.ok) {
            ...
            if (data && Array.isArray(data.metas)) break;
          }
        } catch (_) {}
      }
      if (!data) {
  ...
  useEffect(() => {
    const fetchManifest = async () => {
      try {
        ...
        // 1. Toujours récupérer le manifest local (qui contient Linkzy et Vavoo)
        let localCatalogs: Catalog[] = [];
        const manifestEndpoints = [
          `${API_BASE}/api/livetv/manifest`,
          'https://api.movix.show/api/livetv/manifest',
          'http://localhost:25565/api/livetv/manifest'
        ];
        for (const endpoint of manifestEndpoints) {
          try {
            ...
            if (response.ok) {
              ...
              if (result?.catalogs && Array.isArray(result.catalogs) && result.catalogs.length > 0) {
                localCatalogs = result.catalogs;
                break;
              }
            }
          } catch (e) {
            console.error("Erreur manifest endpoint:", endpoint, e);
          }
        }
        ...
        // 3. Fusionner les catalogues (TV Française officielle + Vavoo + Linkzy)
        const catalogsMap = new Map<string, Catalog>();
        IPTV_ORG_CATALOGS.forEach(c => catalogsMap.set(c.id, c));
        localCatalogs.forEach(c => catalogsMap.set(c.id, c));
        extensionCatalogs.forEach(c => catalogsMap.set(c.id, c));

        const DEFAULT_FALLBACK_CATALOGS: Catalog[] = [
          { type: 'tv', id: 'vavoo_france', name: 'France' },
          { type: 'tv', id: 'vavoo_france-sport', name: 'France Sport' },
          { type: 'tv', id: 'vavoo_all', name: 'Toutes les chaînes' },
          { type: 'tv', id: 'matches_live', name: 'Matches Direct' },
        ];

        let finalCatalogs = Array.from(catalogsMap.values()).filter(c => {
          if (c.id.startsWith('daddylive_')) {
            return c.id === 'daddylive_fr' || c.id === 'daddylive_arabic';
          }
          return true;
        });

        if (finalCatalogs.length === 0) {
          finalCatalogs = DEFAULT_FALLBACK_CATALOGS;
        }
        ...
        if (finalCatalogs.length > 0) {
          const defaultCatalog = finalCatalogs.find(c => c.id === 'iptvorg_fr_general' || c.id === 'iptvorg_fr_all') || finalCatalogs.find(c => c.id === 'vavoo_france') || finalCatalogs[0];
          setSelectedCatalog(defaultCatalog.id);
          setSelectedSource(getSourceKey(defaultCatalog.id));
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchChannels = async () => {
      try {
        ...
        setError(null);
        const channelList = await fetchCatalogChannelsById(selectedCatalog);
    ...
    fetchChannels();
  }, [selectedCatalog, hasAccess, fetchCatalogChannelsById]);
  ...
  // All sources for pill tabs (filter out disabled/removed sources)
  const allSources = useMemo(() => {
    const disabled = new Set(['other', 'livetv', 'daddylive', 'iptv']);
    return availableSources.filter(s => !disabled.has(s));
  }, [availableSources]);
