...
import VirtualTouchKeyboard from './components/VirtualTouchKeyboard';
...
/**
 * Ancien portail de selection de profil.
 *
 * Il n'y a plus de comptes distants ni d'ecran « Qui regarde ? » : un compte
 * local est toujours present des le premier rendu (voir utils/localAccounts).
 * Il ne reste donc rien a garder, et on rend directement l'application.
 */
...
const AppWithIntro: React.FC = () => {
  ...
  // Les pages Hub (YouTube, Twitch) occupent tout l'écran et disposent de leur
  // propre barre de contrôle en bas : la barre de navigation du haut est inutile.
  const isHubRoute = currentPath.startsWith('/hub/');
  const shouldShowHeader = !isWatchRoute && !isWrappedRoute && !isHubRoute;
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <SmoothScroll />
      {/* Glisser-défiler à la souris/stylet (le doigt scrolle déjà nativement) */}
      <DragScroll />
