    ...
    <div style={{ minHeight: 'calc(var(--vh, 1vh) * 100)', overflow: 'hidden' }} className="w-full bg-black text-white overflow-hidden fixed inset-0">
      ...
      {isLoading ? (
        ...
        <div className="w-full h-full flex flex-col items-center justify-center relative">
          ...
          <button
            onClick={() => navigate(`/tv/${encodeId(id!)}`)}
            className="fixed top-6 left-6 z-[50000] flex items-center gap-2 px-3.5 py-2 rounded-xl bg-black/80 hover:bg-black text-white border border-white/20 shadow-2xl backdrop-blur-md transition-all duration-200 cursor-pointer font-medium text-sm"
            title="Quitter (Retour)"
          >
            <svg className="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            <span className="font-semibold text-sm">{t('watch.back', 'Quitter')}</span>
          </button>
