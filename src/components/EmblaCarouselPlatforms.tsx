      ...
      <div className="relative w-full">
        <div className="overflow-visible" ref={emblaRef}>
          <div className="flex gap-6 pr-8 md:pr-16 py-8 pl-4 md:pl-6">
            {items.map((platform) => {
              const isExt = platform.route.startsWith('http://') || platform.route.startsWith('https://');
              const CardInner = (
                <div
                  className="w-full h-full relative bg-white rounded-xl overflow-hidden shadow-lg border border-white/10 group-hover:border-red-500/50 transition-all duration-300"
                  onMouseEnter={() => {
                    if (platform.video && !platform.video.endsWith('.gif')) {
                ...
                >
                  ...
                  />
                  {platform.label && (
                    <p className="absolute bottom-2 left-0 right-0 text-center text-white text-xs font-bold bg-black/75 py-1 px-2 mx-3 rounded-lg backdrop-blur-sm truncate">
                      {platform.label}
                    </p>
                  )}
                  {platform.video && (
                ...
                <div key={platform.id} className="flex-none">
                  {isExt ? (
                    <a
                      ...
                      className="platform-link block w-[250px] h-[150px] group select-none"
                    >
                      {CardInner}
                    </a>
                  ) : (
